...
Отдельно про публичные адреса узлов: сначала они были без них, с выходом через
NAT-шлюз — так дешевле. Это пришлось отменить. Внешний сетевой балансировщик
Yandex сохраняет адрес клиента, узел отвечает ему напрямую, и через шлюз
обратный трафик не проходит — снаружи всё висело до таймаута. Публичные адреса
на узлах тарифицируются, но без них внешний балансировщик не работает.
